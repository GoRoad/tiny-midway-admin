...
<template>
  <AppPage show-footer>
    ...
    <MeModal ref="pwdModalRef" title="重置密码" width="420px" @ok="handlePwdSave()">
      <n-form
        ref="pwdFormRef"
        :model="pwdForm"
        label-placement="left"
      ...
      >
        <n-form-item :label-width="100" label="新密码" path="oldPassword" :rule="required">
          <n-input v-model:value="pwdForm.oldPassword" type="password" placeholder="请输入原密码" />
        </n-form-item>
        <n-form-item :label-width="100" label="确认新密码" path="newPassword" :rule="required">
...
async function handlePwdSave() {
  ...
  await api.changePassword(createPwd(pwdForm.value.newPassword))
