...
export default {
  // 获取用户信息
  getUser: () => request.get('/base/user/detail'),
  // 刷新token
  ...
  // 获取角色权限
  getRolePermissions: () => request.get('/base/user/menu'),
  // 验证菜单路径
  validateMenuPath: path => request.get(`/auth/menu?path=${path}`),
  // 上传文件
  uploadFile:({ action, file, onProgress, categoryId }) => {
    const data = new FormData()
    data.append('file', file)
    categoryId && data.append('categoryId', categoryId)
